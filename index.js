...
app.post('/savedata', urlEncParser, (req, res) => {
    ...
    let str = `"${date}","${req.body.name}","${req.body.state}","${req.body.marks}","${req.body.rev}"\n`;
...
app.get("/list", (req, res) => {
    ...
    csvtojson({headers:['date','name','state','marks','rev']}).fromFile(path.join(__dirname, 'data/records.csv'))
    .then(data => {
        res.render('index', {title: "List of games", records: data});
    })
    .catch(err => {
